...
# Modified by WahyuBudiman (add more scripts for Cloudnative-pg)
...
RPTFILE=${CURDIR}/${PREFIX}_hc.md
TMPRPTFILE=${CURDIR}/${PREFIX}_hc.log.tmp
...
if [ -f ${TMPRPTFILE} ]; then
   rm -f ${TMPRPTFILE}
fi

touch ${TMPRPTFILE}
echo "Healthcheck on $(date)" > ${TMPRPTFILE}
echo "On $(hostname -a)" >> ${TMPRPTFILE}
echo "IP Address $(hostname -i)" >> ${TMPRPTFILE}
echo "PGHOST: ${PGHOST}" >> ${TMPRPTFILE}
echo "PGPORT: ${PGPORT}" >> ${TMPRPTFILE}
echo "PGUSR:  ${PGUSR}" >> ${TMPRPTFILE}
echo "DBNAME: ${DBNAME}" >> ${TMPRPTFILE}
echo " " >> ${TMPRPTFILE}
echo " " >> ${TMPRPTFILE}
...
parse_to_markdown() {
    ...
    host_name=$(hostname)
    local ip_addr
    ip_addr=$(hostname -i 2>/dev/null || ip route get 1 2>/dev/null | awk '{print $7}' || echo "127.0.0.1")
    
    awk -v gdate="${gen_date}" -v hname="${host_name}" -v ipaddr="${ip_addr}" \
        -v pghost="${PGHOST}" -v pgport="${PGPORT}" -v pgusr="${PGUSR}" -v dbname="${DBNAME}" '
    BEGIN {
    ...
    }
    {
        ...
        if (stripped ~ /^[ \t]*--[0-9]/) {
            ...
            space_idx = index(val, " ")
            if (space_idx > 0) {
                num_part = substr(val, 1, space_idx - 1)
                title_part = substr(val, space_idx + 1)
            } else {
                num_part = val
                title_part = ""
            }
            ...
            print hashes " " num_part " " title_part
            print ""
            next
        }
...
# Convert temporary report to Markdown
echo "Formatting report to Markdown..."
parse_to_markdown "${TMPRPTFILE}" "${RPTFILE}"
rm -f "${TMPRPTFILE}"
